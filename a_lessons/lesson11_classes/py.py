...
class PrimaryGroup(Group):
    ...
    #селф это ссылка на экземпляр, который мы создаем. то есть вместо селф можно представлять название 
    # экземпляра который мы только что создали. без селф думаю нельзя тк логика его предусматривает. 
    # плюс как я понял что происходит под капотом. мы создаем какой-то экземпляр с названием и этому 
    # экземпляру даем характеристики которые присущи только ему. это все инициализируется полд капотом и 
    # под капотом где-то создается наш элемент с его характеристиками. без селф непонятно какому экземпляру 
    # какие характеристики присваиваются
    def moove(self):
        print('Run fast')
